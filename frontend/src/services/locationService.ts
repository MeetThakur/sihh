...
class LocationService {
  private readonly API_ENDPOINTS = {
    ...
    cities: (stateCode: string) => `https://api.countrystatecity.in/v1/countries/IN/states/${stateCode}/cities`,

    // Backup API for pincode lookup
    pincode: (pincode: string) => `https://api.postalpincode.in/pincode/${pincode}`,

    // Alternative API
    alternativeStates: 'https://cdn-api.co-vin.in/api/v2/admin/location/states',
    alternativeDistricts: (stateId: string) => `https://cdn-api.co-vin.in/api/v2/admin/location/districts/${stateId}`,
  };
  ...
  getStateByNameOrCode(nameOrCode: string): State | null {
    return this.INDIAN_STATES.find(
      state =>
